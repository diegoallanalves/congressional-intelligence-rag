...
def load_v10_export(
    ...
    missing = REQUIRED_SHEETS.difference(
        xls.sheet_names
    )

    if missing:
        raise ValueError(
            f"{name}: missing required sheets "
            f"{sorted(missing)}"
        )
    ...
    file_congress = congress_from_filename(
        name
    )
    ...
    actions.loc[
        actions["Congress"] == "Unknown",
        "Congress",
    ] = file_congress

    evidence.loc[
        evidence["Congress"] == "Unknown",
        "Congress",
    ] = file_congress
    ...
    # Country is not present in the V10 Evidence sheet.
    # Map it from the corresponding Bill + Congress.
    lookup = (
        actions[
            [
                "Bill",
...
def merge_v10_exports(
    ...
    for name, blob in sources:
        ...
        actions, evidence = load_v10_export(
            name,
            blob,
        )
    ...
    # Prefer successful and relevant records when
    # the same action exists in multiple uploads.
...
def score_action(row):
    # Evidence is the primary NLP source.
    # Mechanism and summary are fallback context.
    evidence_text = (
        safe_text(
            row.get("Evidence Quotes")
        )
        + " "
        + safe_text(
            row.get(
    ...
    # Small adjustments from the original RAG
    # classification while keeping evidence primary.
    if (
        safe_text(
            row.get("Relevant")
        ).upper()
...
def build_intelligence(
    ...
    out["Matched Evidence Terms"] = (
        terms
    )

    out["Confidence Numeric"] = (
        out["Confidence"]
        .apply(confidence_number)
    )
        ...
        blocks.append(
            "\n".join(
                [
                    ...
                    (
                        f"CLASSIFICATION: "
                        f"{row.get('Primary Classification', '')}"
                    ),
                    (
                        f"EXCLUSION INDEX: "
                        f"{row.get('China Exclusion Index', 0)}"
                    ),
                    (
                        f"THEMES: "
                        f"{row.get('NLP Themes', '')}"
                    ),
                    (
                        "EVIDENCE: "
                        + " | ".join(
